...
fn is_pid_alive_raw(pid: libc::pid_t) -> bool {
    // SAFETY: kill with signal 0 does not send a signal and is the POSIX liveness probe.
...
fn spawn_proxy_socket_dir_cleanup_worker(
    ...
    // SAFETY: fork is called in this short-lived helper context before spawning user code.
    ...
    if pid == 0 {
        ...
        // SAFETY: child process exits immediately without running Rust destructors.
        unsafe { libc::_exit(0) };
    }
...
fn spawn_host_bridge(endpoint: SocketAddr, uds_path: &Path) -> io::Result<libc::pid_t> {
    ...
    close_fd(write_fd)?;
    // SAFETY: read_fd is a valid owned file descriptor from create_ready_pipe.
...
fn run_host_bridge(endpoint: SocketAddr, uds_path: &Path, ready_fd: libc::c_int) -> io::Result<()> {
    ...
    // SAFETY: ready_fd is a valid owned file descriptor from create_ready_pipe.
...
fn ensure_loopback_interface_up() -> io::Result<()> {
    // SAFETY: libc::socket is called with valid arguments for a datagram IPv4 socket.
    let fd = unsafe { libc::socket(libc::AF_INET, libc::SOCK_DGRAM | libc::SOCK_CLOEXEC, 0) };
    if fd < 0 {
        return Err(io::Error::last_os_error());
    }

    // SAFETY: zeroed ifreq is a valid initialization pattern.
    ...
    // SAFETY: fd is valid and ifreq points to writable memory.
    let read_flags_result = unsafe { libc::ioctl(fd, libc::SIOCGIFFLAGS as _, &mut ifreq) };
    ...
    // SAFETY: union field is populated by SIOCGIFFLAGS.
    ...
    if (current_flags & up_flag) != up_flag {
        ...
        // SAFETY: fd is valid and ifreq contains the desired flag updates.
        let set_flags_result = unsafe { libc::ioctl(fd, libc::SIOCSIFFLAGS as _, &ifreq) };
    ...
    // SAFETY: sockaddr_in can be reinterpreted as sockaddr for ifreq assignment.
    unsafe {
        addr_req.ifr_ifru.ifru_addr =
            *(&loopback_addr as *const libc::sockaddr_in as *const libc::sockaddr);
    }
    // SAFETY: fd is valid and addr_req contains loopback address.
    let set_addr_result = unsafe { libc::ioctl(fd, libc::SIOCSIFADDR as _, &addr_req) };
...
fn set_parent_death_signal() -> io::Result<()> {
    // SAFETY: PR_SET_PDEATHSIG configures process behavior and does not access invalid memory.
...
fn close_fd(fd: libc::c_int) -> io::Result<()> {
    // SAFETY: fd was created/opened by this process.
