...
- `src/server/response.rs` is the server-owned response finalizer. It separates worker-originated text from server-only notices, creates transcript spill files for oversized text-only replies, creates spill bundles with `transcript.txt`, `events.log`, and `images/` for oversized mixed or image-heavy replies, and decides the bounded inline preview at seal time.
